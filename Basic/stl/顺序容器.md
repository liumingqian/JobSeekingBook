...
//只是预留了空间，设置了capacity，并没实际分配内存，可以减少vector增长造成的拷贝
v.reserve();

//构造：emplace_back能在vector当前末尾就地通过参数构造对象，
...
连续存储的双端可变长数组。在功能上合并了list和vector，但占用内存比较大。
  ...
  `deque` 的元素不一定是相邻存储的：典型实现是采用单独分配的固定大小的相邻元素数组存储数据，并维护一个指向各个连续内存空间的指针数组，所以它的迭代器势必也要能够判断各个连续空间的边界以便于知道自己该如何进行`++`和`--`

## priority\_queue

默认存储元素的容器是vector，priority\_queue无法遍历，默认建立的是大根堆。

```cpp
//priority_queue模板有三个参数
...
stack和queue是两种限定操作的容器适配器，底层一般用list或deque实现。queue的容器需要支持以下的方法：
